...
public class HelloKafkaController {
    ...
    public String hello() throws Exception {
        ...
        for(int i=0;i<messagesPerRequest;i++) {
        	keyCount = keyCount + 1;
        	this.template.send(topicName, String.valueOf(keyCount),
    ...
    public void listenasString(ConsumerRecord<String, String> cr,
                               @Payload String payload) throws InterruptedException {
    ...
    public void listenAsByteArray(ConsumerRecord<String, byte[]> cr,
                                  @Payload byte[] payload) throws InterruptedException {
        logger.info("Logger 2 [ByteArray] received key {}: Type [{}] | Payload: {} | Record: {}", cr.key(),
                typeIdHeader(cr.headers()), payload, cr.toString());
        Thread.sleep(360000);
        latch.countDown();
    }
